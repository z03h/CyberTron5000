...
async def load(ctx, extension=None):
    if not extension:
        ...
        await ctx.send("\n".join(
            [f":arrow_up: `cogs.{f[:-3]}`" for f in os.listdir('cogs') if
             f.endswith(".py")]))
...
async def unload(ctx, extension=None):
    if not extension:
        ...
        await ctx.send("\n".join(
            [f":arrow_down: `cogs.{f[:-3]}`" for f in os.listdir('cogs') if
             f.endswith(".py")]))
...
async def reload(ctx, extension=None):
    if not extension:
        ...
        await ctx.send("\n".join(
            [f":repeat: `cogs.{f[:-3]}`" for f in os.listdir('cogs') if f.endswith(".py")]))
